...
public class ManagedNodesPositioner {
    private static Logger log = Logger.getLogger(ManagedNodesPositioner.class);
    ...
    private void checkInitValues() {
        ...
        if(tileLengthCm < 5) { throw new IllegalArgumentException("Tile-length must be at least 5"); }
        if(lengthCm <= tileLengthCm) { throw new IllegalArgumentException("Length "+lengthCm+" must be greater than tile length of "+tileLengthCm); }
        if(widthCm <= tileLengthCm) { throw new IllegalArgumentException("Width "+widthCm+" must be greater than tile length of "+tileLengthCm); }
        if(!analysis.getExtendedNodeMap().containsKey(range)) {throw new IllegalArgumentException("Analysis does not contain any info on freq range "+range);}
    }
    ...
    private void initializeMatrix() {
        ...
        if(data.getManagedNodesMacMap().keySet().size() < 2) {throw new IllegalArgumentException("We need at least 3 active nodes. Found only "+data.getManagedNodesMacMap().keySet());}
