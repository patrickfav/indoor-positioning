...
public class ExtendedNodeTrilaterationPositioner implements Callable<ExtendedNodeTrilaterationPositioner.ExtendedNodePositionWrapper> {
    private static Logger log = Logger.getLogger(ExtendedNodeTrilaterationPositioner.class);
    ...
    public ExtendedNodePositionWrapper calculatePosition() {
        ...
        if(data.getExtendedNodesMacList().get(macExtendedNode).isIgnored()) {
            log("info","Ignoring "+macExtendedNode+" because it is on the blacklist",wrapper); return returnAndNotify(wrapper);
        }

        if(macThatSeeExtendedNode.size() < config.getMinVisibilityNeededForTrilateration()) {
            log("info","Ignoring "+macExtendedNode+" because it is only seen by "+macThatSeeExtendedNode.size(),wrapper); return returnAndNotify(wrapper);
        }
        ...
        if(validVisibilityCount < config.getMinVisibilityNeededForTrilateration()) {
            log("info","Ignoring "+macExtendedNode+" because it is only seen by "+validVisibilityCount+" with over "+config.getMinDbmNeededToConsiderForTrilateration()+"dBm",wrapper); return returnAndNotify(wrapper);
        }
        ...
        if(mostLikely == null) {
            log("info","No position found with trilateration for " + macExtendedNode,wrapper); return returnAndNotify(wrapper);
        } else {
            log("info","Position found with trilateration for " + macExtendedNode+": "+mostLikely.getSelectedPoint().getX()+","+mostLikely.getSelectedPoint().getY()+" out of "+mostLikely.getAllPossiblePoints().size()+" with same probability "+mostLikely.getMaxProbability(),wrapper);
        }
